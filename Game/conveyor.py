...
class Conveyor:
    ...
    def speed(self, speed: float):
        if not isinstance(speed, (int, float)): raise TypeError("speed must be a number")
        self.__speed = speed
    ...
    def draw(self):
        ...
        for conv in range(self.length):
            ...
            x_pos = int(self.x) + (conv * sprite_width)
            pyxel.blt(x_pos, int(self.y), *constants.CONVEYOR_SPRITE)